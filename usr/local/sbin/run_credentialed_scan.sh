...
CREDENTIAL_RESPONSE_CODE=$(
  echo "${CRED_ID}" \
  | grep -oE "create_credential_response status=.*" \
  | cut -d "\"" -f2
)

# If we didn't get a 2xx
if [[ ${CREDENTIAL_RESPONSE_CODE:0:1} -ne 2 ]]
then
  echo "Failed to create credential, quitting..."
  exit 1
fi
...
echo "Credential ID is ${CRED_ID}"

# Create a target, using the credentials we created above
TARGET_ID=$(
  gvm-cli \
    socket \
    --socketpath ${SOCKET_PATH} \
